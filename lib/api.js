...
/** Xoá 1 xe — nhờ ON DELETE CASCADE (step9), toàn bộ parts,
 * maintenance_logs, fuel_logs của xe này sẽ tự động bị xoá theo. */
...
// Danh sách phụ tùng phổ biến cho xe máy nói chung — dùng để tạo sẵn khi
// thêm xe mới, tránh người dùng phải bấm "Thêm phụ tùng" từng cái một.
// Chu kỳ chỉ mang tính tham khảo chung, người dùng tự sửa lại cho đúng
// với xe của mình (hoặc xoá nếu không cần, VD: xe không có nhông sên dĩa).
export const DEFAULT_PART_TEMPLATES = [
  { name: "Nhớt máy", intervalKm: 1500, intervalMonths: 3 },
  { name: "Lọc gió", intervalKm: 8000, intervalMonths: 12 },
  { name: "Bugi", intervalKm: 8000, intervalMonths: 12 },
  { name: "Nhông sên dĩa", intervalKm: 10000, intervalMonths: null },
  { name: "Dầu phanh", intervalKm: null, intervalMonths: 24 },
  { name: "Lốp trước", intervalKm: 15000, intervalMonths: null },
  { name: "Lốp sau", intervalKm: 12000, intervalMonths: null },
  { name: "Ắc quy", intervalKm: null, intervalMonths: 24 },
];

/** Tạo sẵn bộ phụ tùng mặc định cho xe mới, tất cả bắt đầu tính từ ODO hiện
 * tại của xe (currentOdo) và ngày hôm nay — 1 lệnh insert hàng loạt, nhanh
 * hơn gọi addPart() nhiều lần. */
export async function addDefaultParts(vehicleId, currentOdo = 0) {
  const today = new Date().toISOString().slice(0, 10);
  const rows = DEFAULT_PART_TEMPLATES.map((tpl, index) => ({
    vehicle_id: vehicleId,
    name: tpl.name,
    interval_km: tpl.intervalKm,
    interval_months: tpl.intervalMonths,
...
/** Hoán đổi sort_order giữa 2 phụ tùng liền kề (dùng cho nút lên/xuống) */
export async function swapPartOrder(partA, partB) {
  const { error: e1 } = await supabase
    .from("parts")
    .update({ sort_order: partB.sort_order })
    .eq("id", partA.id);
  if (e1) throw e1;

  const { error: e2 } = await supabase
    .from("parts")
    .update({ sort_order: partA.sort_order })
    .eq("id", partB.id);
  if (e2) throw e2;
}
